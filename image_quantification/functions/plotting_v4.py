...
# @Last Modified time: 2020-07-16 04:06:38
...
def get_target_grid_polar(rel_points):
	...
	target_grid_polar = {}
	for t in ['c', 'l', 'h']:
		T0 = np.array((0,-rel_points[f'T0{t}']))
		T2 = np.array((aT0T2, rel_points[f'T2{t}']))
		T3 = np.array((aT0T3, rel_points[f'T3{t}']))
		T4 = np.array((aT0T4, rel_points[f'T4{t}']))
		T5 = np.array((aT0T5, rel_points[f'T5{t}']))
		T3_ = np.array((aT0T3_, rel_points[f'T7{t}']))
		C0 = np.array((0, rel_points['center']))
		target_grid_polar[t] = np.stack((T0, T2, T3, T4, T5, T3_, C0), axis = 0)
	# print(target_grid_polar)
...
def get_polar_plot_values(analysis_params, channel_no, matrix, cmap, rel_points):
	...
	target_grid_polar = get_target_grid_polar(rel_points)
	targetGridPos = get_target_grid_cart()
	# print(targetGridPos)
...
def get_imshow_values(bundles_df, bundle_no, image, r_z, z_offset, channel_no):
	...
	_, coord_Te1s = my_help.get_target_v4(bundle_no, bundles_df, return_type = 'e1')
	_, coord_Te2s = my_help.get_target_v4(bundle_no, bundles_df, return_type = 'e2')
	_, target_ellipses = my_help.get_target_v4(bundle_no, bundles_df, return_type = 'ellipse')
...
""" ========================= Plotting =========================== """
def plot_individual_bundles_v4(bundle_no, bundles_df, image_norm, xRatio, yRatio, **kwargs):
	## unravel params
	if('figsize' in kwargs.keys()):
		figsize = kwargs['figsize']
	else:
		figsize = (18,5)
	if('num_cols' in kwargs.keys()):
	...
	matching_info = settings.matching_info
	channel_mapping = matching_info.channel_mapping
	num_subplots = len(channel_mapping.keys())
	
	### R heels info
	r_z = int(bundles_df.loc[bundle_no,'coord_Z_R4']) - 1
	r_heel_coords = my_help.get_heel_coords(bundle_no, bundles_df)
	r_center = my_help.get_bundle_center(bundle_no, bundles_df)
	...
	ind_targets, target_ellipses = my_help.get_target_v4(bundle_no, bundles_df, return_type = 'ellipse')
	coordR4s = my_help.get_rx_coords(bundle_no, bundles_df, ind_targets, 4)
	coords = np.concatenate((coord_Tcs, coord_Te1s, coord_Te2s))
	
	### c
	c = my_help.line_intersection((r_heel_coords[2,:], coord_Tcs[2,:]), (r_heel_coords[3,:], coord_Tcs[5,:]))
	...
	### plot
	fig, axes = plt.subplots(int(np.ceil(num_subplots/num_cols)), num_cols, figsize = figsize)
	...
	for plt_i in range(image_norm.shape[-1]):
		...
		for i in range(6):
			ax.plot(r_heel_coords[i,0]-xlim[0], r_heel_coords[i,1]-ylim[0], 
					color = matching_info.color_code[i+1], marker = 'o', markersize=12)
		
		# plot center
		ax.plot(r_center[0,0]-xlim[0], r_center[0,1]-ylim[0], 
					color = matching_info.color_code[0], marker = 'o', markersize=12)

		# plot targets and extended targets
		if(channel_mapping[plt_i] == 'FasII'):
			for i in matching_info.index_to_target_id.keys():
				if(kwargs['is_plot_extension']):
					ax.plot(coord_Te1s[i,0]-xlim[0], coord_Te1s[i,1]-ylim[0],
						marker = '*', color = matching_info.color_code[matching_info.index_to_target_id[i]], 
						markersize=8)
					ax.plot(coord_Te2s[i,0]-xlim[0], coord_Te2s[i,1]-ylim[0],
						marker = '.', color = matching_info.color_code[matching_info.index_to_target_id[i]], 
						markersize=8)
				ax.add_patch(Ellipse((coord_Tcs[i,0]-xlim[0] , coord_Tcs[i,1]-ylim[0]),
									 ...
									 linewidth=2))
		for i in matching_info.index_to_target_id.keys():
			ax.plot(coord_Tcs[i,0]-xlim[0], coord_Tcs[i,1]-ylim[0],
				marker = 'o', mec = matching_info.color_code[matching_info.index_to_target_id[i]], 
				markersize=8, mew = 2.0, mfc = 'none')
		
		# plot lines
		if(kwargs['is_plot_line']):
			# c-T3
			ax.plot([c[0]-xlim[0], coord_Tcs[2,0]-xlim[0]], 
					[c[1]-ylim[0], coord_Tcs[2,1]-ylim[0]], 
					'--', alpha = 0.8, color = matching_info.color_code[3], linewidth = 4)
			# c-T7
			ax.plot([c[0]-xlim[0], coord_Tcs[5,0]-xlim[0]], 
					[c[1]-ylim[0], coord_Tcs[5,1]-ylim[0]], 
					'--', alpha = 0.8, color = matching_info.color_code[4], linewidth = 4)       
		# plot image
		ax.imshow(bundle_img[:,:,plt_i], cmap=plt.cm.gray)

		# plot axis
		ax.set_title(channel_mapping[plt_i], fontsize=16)
...
def plot_bundle_vs_matrix_gr_v4(bundle_no, bundles_df, image, intensity_matrix, fig_params, tick_params, plot_options, **kwargs):
	## parameters from settings
	paths = settings.paths
	analysis_params_general = settings.analysis_params_general
	matching_info = settings.matching_info
	...
	thrs, thr_names, num_norm_channels = plot_options
	...
	if(kwargs['is_save']):
		plt.ioff()
		subfolder1 = 'HeatMap'
		subfolder2 = 'slice_type_' + str(analysis_params_general.slice_type) + 'center_type' + str(analysis_params_general.center_type) + '_GFPRFP'
...
def plot_bundle_vs_matrix_others_v4(bundle_no, bundles_df, image, intensity_matrix, fig_params, tick_params, plot_options, **kwargs):
	## parameters from settings
	paths = settings.paths
	analysis_params_general = settings.analysis_params_general
	matching_info = settings.matching_info
...
def plot_polar_v4(bundle_no, bundles_df, image, channel_no, matrix, fig_params, rel_points, **kwargs):
	...
	if(analysis_params_general.center_type == 0):
		ax2.plot([target_grid_polar['c'][0,0], target_grid_polar['c'][matching_info.target_id_to_index[3],0]], [target_grid_polar['c'][0,1], target_grid_polar['c'][matching_info.target_id_to_index[3],1]], '--', color = '0.5')
		ax2.plot([target_grid_polar['c'][0,0], target_grid_polar['c'][matching_info.target_id_to_index[7],0]], [target_grid_polar['c'][0,1], target_grid_polar['c'][matching_info.target_id_to_index[7],1]], '--', color = '0.5')
	elif(analysis_params_general.center_type == 1):
	...
	## plot angle reference
	if(analysis_params_general.center_type == 0):
		ax3.plot([target_grid_polar['c'][0,0], target_grid_polar['c'][matching_info.target_id_to_index[3],0]], [target_grid_polar['c'][0,1], target_grid_polar['c'][matching_info.target_id_to_index[3],1]], '--', color = '0.5')
		ax3.plot([target_grid_polar['c'][0,0], target_grid_polar['c'][matching_info.target_id_to_index[7],0]], [target_grid_polar['c'][0,1], target_grid_polar['c'][matching_info.target_id_to_index[7],1]], '--', color = '0.5')
	elif(analysis_params_general.center_type == 1):
	...
	if(is_save):
		...
		subfolder2 = f'slice_type_{analysis_params_general.slice_type}_acenter_type_{analysis_params_general.center_type}'
