...
# @Last Modified time: 2020-04-14 16:26:29
...
def get_target_grid_polar_new(rel_points):
    ...
    T0 = np.array((0,-rel_points['T0']))
    T2 = np.array((aT0T2, rel_points['T2']))
    T3 = np.array((aT0T3, rel_points['T3']))
    T4 = np.array((aT0T4, rel_points['T4']))
    T5 = np.array((aT0T5, rel_points['T5']))
    T3_ = np.array((aT0T3_, rel_points['T7']))
    C0 = np.array((0, rel_points['center']))
    
    target_grid_polar = np.stack((T0, T2, T3, T4, T5, T3_, C0), axis = 0)

    T3_etd = np.array((aT0T3, rel_points['T3_etd']))
    T4_etd = np.array((aT0T4, rel_points['T4_etd']))
    T7_etd = np.array((aT0T3_, rel_points['T7_etd']))

    target_grid_polar_etd = np.stack((T0, T2, T3_etd, T4_etd, T5, T7_etd, C0), axis = 0)
    
    return target_grid_polar, target_grid_polar_etd
...
def get_polar_plot_values(analysis_params, channel_no, matrix, cmap, rel_points):
    ...
    target_grid_polar, target_grid_polar_etd = get_target_grid_polar_new(rel_points)
    ...
    return thetav, rv, z, norm, target_grid_polar, target_grid_polar_etd
...
def get_imshow_values(bundles_df, bundle_no, image, r_z, z_offset, channel_no):
    ...
    coords = np.concatenate((coord_targets, coord_targets_extended))
    ...
    return r_heel_coords, coord_targets, coord_targets_extended, bundle_img, xlim, ylim
...
def plot_individual_bundles(bundle_no, bundles_df, image_norm, xRatio, yRatio, **kwarg):
    ...
    matching_info = settings.matching_info
    num_subplots = len(matching_info.channel_mapping.keys())/2
    ...
    ### targets info
    ind_targets, coord_targets = my_help.get_target_coords(bundle_no, bundles_df)
    ind_targets, coord_target_extends = my_help.get_target_coords(bundle_no, bundles_df, return_type = 'extended')
    coordR4s = my_help.get_rx_coords(bundle_no, bundles_df, ind_targets, 4)
    coords = np.concatenate((coord_targets, coord_target_extends))
    ...
    ### plot
    fig, axes = plt.subplots(int(np.ceil(num_subplots/4)), 4, figsize = (18,8))
    ...
    for plt_i in range(image_norm.shape[-1]):
        ...
        # plot extended targets
        for i in matching_info.index_to_target_id.keys():
            ax.plot(coord_target_extends[i,0]-xlim[0], coord_target_extends[i,1]-ylim[0],'*', mec = matching_info.color_code[matching_info.index_to_target_id[i]], markersize=8, mew = 2.0, mfc = 'none')
...
def plot_bundle_vs_matrix_all(bundle_no, bundles_df, image, intensity_matrix, fig_params, tick_params, plot_options, **kwarg):
    ...
    for ax in axes.flat:
        ...
        elif(i_fig%3 == 1): # plot max intensity            
                
            ### matrix and thresholding mask
            matrix1 = np.max(intensity_matrix[channel_no,:,:,:], axis = 2)
            
            vmax = np.percentile(matrix1.flatten()[matrix1.flatten()>=0], 99)
        ...
        elif(i_fig%3 == 2): # plot mean intensity            
            
            ### matrix
            matrix2 = np.mean(intensity_matrix[channel_no,:,:,:], axis = 2)
            vmax = np.percentile(matrix2.flatten()[matrix2.flatten()>=0], 100)
...
def plot_polar(bundle_no, bundles_df, image, channel_no, matrix, fig_params, rel_points, **kwarg):
    ...
    if('is_label_off' in kwarg.keys()):
        is_label_off = kwarg['is_label_off']
    else:
        is_label_off = False
    if('is_save' in kwarg.keys()):
        is_save = kwarg['is_save']
        ...
        is_save = False
    if('is_extended_target' in kwarg.keys()):
        is_extended_target = kwarg['is_extended_target']
    else:
    ...
    #### subfigure 1
    r_heel_coords, coord_targets, coord_targets_extended, bundle_img, xlim, ylim = get_imshow_values(bundles_df, bundle_no, image, r_z, analysis_params_general.z_offset, channel_no)
    #### subfigure2: max projection of z
    mm = np.max(matrix[channel_no, :, :, :], axis = 2)
    thetav, rv, z1, norm1, target_grid_polar, target_grid_polar_etd = get_polar_plot_values(analysis_params, channel_no, mm, colormap, rel_points)
    #### subfigure3: mean over z
    mm = np.mean(matrix[channel_no, :, :, :], axis = 2)
    thetav, rv, z2, norm2, target_grid_polar, target_grid_polar_etd = get_polar_plot_values(analysis_params, channel_no, mm, colormap, rel_points)
    ...
    for i in matching_info.index_to_target_id.keys():
        ...
        if(is_extended_target):
            ax.plot(coord_targets_extended[i,0]-xlim[0], coord_targets_extended[i,1]-ylim[0],'*', mec = matching_info.color_code[matching_info.index_to_target_id[i]], markersize=5, mew = 2.0, mfc = 'none')
    ...
    if(is_extended_target):
        for i in [2,3,5]:
            ax2.plot(target_grid_polar_etd[i,0], target_grid_polar_etd[i,1], '*', color = matching_info.color_code[matching_info.index_to_target_id[i]], markersize = 10, mew = 3, mfc = 'none')
    ...
    if(is_extended_target):
        for i in [2,3,5]:
            ax3.plot(target_grid_polar_etd[i,0], target_grid_polar_etd[i,1], '*', color = matching_info.color_code[matching_info.index_to_target_id[i]], markersize = 10, mew = 3, mfc = 'none')
...
def plot_polar_new(bundle_no, bundles_df, image, channel_no, matrix, fig_params, rel_points, **kwarg):
    ...
    mask = z2 ==0
    zm = ma.masked_array(z2, mask=mask)
    vmax = np.percentile(zm, 99)
    sc = ax3.pcolormesh(thetav, rv, zm, cmap=colormap, vmin = 0, vmax = vmax)
