...
# @Last Modified time: 2020-08-07 02:50:54
...
- **kwargs: is_print, is_plot
Output: 
- params: parameters to passed on for intensity calculation function -- z_offset, num_x_section, r_z, phis, slice_center, y_ticks, radius
...
- rel_points: relative coordinates for target positions and heel positions
"""
def get_slice_params_v3(bundles_df, bundle_params, img_name, **kwargs):
	### decomposite parameters.
	analysis_params_general = settings.analysis_params_general
	radius_expanse_ratio = analysis_params_general.radius_expanse_ratio[analysis_params_general.center_type]
	target_id_to_index = settings.matching_info.target_id_to_index
...
def get_intensity_matrix_new(params, image):
	...
	# print('\n Before adjustment:')
	# print(f'x: min {np.min(vxf)}, max {np.max(vxf)}')
	# print(f'y: min {np.min(vyf)}, max {np.max(vyf)}')
	# print(f'image shape: {matrix_shape}')
	...
	if(max(ybound) >= matrix_shape[0]):
		...
		# print(sum(ybound >= matrix_shape[0]))
	...
	# print(f'x: min {np.min(vxf)}; max {np.max(vxf)}')
	# print(f'y: min {np.min(vyf)}; max {np.max(vyf)}')
	# print(f'image boundary: {ix_min}:{ix_max}, {iy_min}:{iy_max}')
