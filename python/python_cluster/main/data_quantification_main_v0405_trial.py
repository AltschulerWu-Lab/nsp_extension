...
# @Last Modified time: 2020-04-05 19:26:38
...
import intensity_calculation15 as my_int
import parse_bundle as my_pb
import plotting11 as my_plot
...
def analyze_image(bundles_df, annot_bundles_df, image_norm, image_name):
	...
	ind_part = int(len(annot_bundles_df.index)/3)
	...
	intensity_matrix = np.zeros((len(annot_bundles_df.index[:ind_part]), num_norm_channels, matrix_y, matrix_x, matrix_z))
...
def save_results(annot_bundles_df, intensity_matrix, params, rel_points):
	...
	output_data = {
		...
		'bundle_nos':list(annot_bundles_df.index[:ind_part]),   
	}

	now = datetime.datetime.now()
	date_info = str(now.year)+str(now.month)+str(now.day)
	output_name = f'{category_id}_{time_id}hrs_sample{sample_id}_region{region_id}_slice{analysis_params_general.slice_type}_center{analysis_params_general.center_type}_{channels_type}_v{date_info}_02.pickle'
