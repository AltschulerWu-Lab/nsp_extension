# -*- coding: utf-8 -*-
# @Author: sf942274
# @Date:   2019-11-15 17:31:35
# @Last Modified by:   sf942274
# @Last Modified time: 2019-11-15 17:46:12
...
def set_parameters():
    ...
    ### directory
    data_prefix = '/awlab/projects/2015_07_Neural_Superposition/Projects/Weiyue/Data_Analysis/Fate_Switching_experiments/Data_Gal80'
    fig_out_prefix = '/awlab/projects/2015_07_Neural_Superposition/Projects/Weiyue/Data_Analysis/Fate_Switching_experiments/Output_Gal80/FigureOutput_v0812_long'
    data_out_prefix = '/awlab/projects/2015_07_Neural_Superposition/Projects/Weiyue/Data_Analysis/Fate_Switching_experiments/Output_Gal80/DataOutput_v0812_long'
    log_prefix = '/awlab/projects/2015_07_Neural_Superposition/Projects/Weiyue/Code/python_cluster/log_txts'
    ...
    paths = [image_path, roi_path, annot_path, log_path, fig_out_prefix, data_out_prefix]
    ...
    analysisParams_general = (num_angleSection, num_outsideAngle, num_Xsection, z_offset, radius_expanse_ratio, slice_type, center_type)
    ...
    print_to_log(log_path, "=====" + annot_name + " Analysis Start! =====")

    return paths, analysisParams_general, matching_info, scale_factor

def print_to_log(log_path, info):
...
def import_data(paths):
    ...
    summary_df = pd.read_csv(annot_path)
    ...
    roi_df = pd.read_csv(os.path.join(roi_path, roi_name))
    ...
    image = img_as_float(skiIo.imread(os.path.join(image_path, image_name)))
...
def process_annotation(roi_df, annot_df, m2p_ratio):
    ...
    if(annotation_type == 1):
        bundles_df = my_pb.getBundlesInfo_v1(roi_df, annot_df, m2p_ratio[0], m2p_ratio[1], is_extended_target_list)
    elif(annotation_type == 2):
        bundles_df = my_pb.getBundlesInfo_v2(roi_df, annot_df, m2p_ratio[0], m2p_ratio[1], is_extended_target_list)
...
def process_image(image, image_shape, log_path, channel_cmap, scale_factor):
    ### number of channels
    nChannels = image.shape[3]
    num_norm_channels = len(channel_cmap.keys()) # number of channels of normalized image

    print("4 channels!")
    print_to_log(log_path, "4 channels!")
    ...
    print("R3/R4 v2")
    print_to_log(log_path, "R3/R4 v2")
...
def analyze_image(bundles_df, annot_bundles_df, paths, analysisParams_general, matching_info, image_norm, image_name):
    ### parameters
    image_path, roi_path, annot_path, log_path, fig_out_prefix, data_out_prefix = paths
    num_angleSection, num_outsideAngle, num_Xsection, z_offset, radius_expanse_ratio, slice_type, center_type = analysisParams_general
    targetIndexMatch, ColorCode, channel_mapping, channel_cmap, targetIndexMatch_rev = matching_info

    ### initialization
    print('-----' + image_name + '------')
    print_to_log(log_path, '-----' + image_name + '------')
    ...
    print_to_log(log_path, "total time: " + str(time_dur))
    ...
    for ind, bundle_No in enumerate(annot_bundles_df.index):
        ...
        analysis_params = [analysisParams_general[0], analysisParams_general[1], analysisParams_general[2], analysisParams_general[3], analysisParams_general[4][center_type]]
        bundleParams = [bundle_No, indTs, coordTs, coord_Center, SliceZeroPoint, SliceOnePoint, LengthOnePoint, CenterPoints[center_type], Rcell_nums[center_type]]
        if(slice_type == 0):
        ...
        time_start = time.time()
        for channelNo in range(num_norm_channels):
            print_to_log(log_path, "Channle No: " + str(channelNo))
...
def print_results(bundles_df, annot_bundles_df, IntensityMatrix, params, rel_points, paths, analysisParams_general, matching_info, image_norm, img_name, thr_otsu, thr_li, thr_isodata):
    ...
    for ind, bundle_No in enumerate(annot_bundles_df.index):
        ...
        if(len(matrix.flatten()) > 0):
            ...
            ## heat map
            plt.ioff()
            ori_X = np.round(np.linspace(0, radius_expanse_ratio[center_type], matrix.shape[2]), 2)
            ...
            for thrFunction in [0, 1, 2, 3]: # different thresholding methods
                ...
                figParams = [center_type, pp_i, fig_out_prefix, img_name, figname, slice_type, radius_expanse_ratio[center_type]]
            ...
            ## polar plot
            figParams = pp_i, fig_out_prefix, img_name, radius_expanse_ratio[center_type], center_type, slice_type
...
def save_results(annot_bundles_df, IntensityMatrix, params, rel_points, paths, slice_type, center_type):
    image_path, roi_path, annot_path, log_path, fig_out_prefix, data_out_prefix = paths
    ...
    outputname = f'{categoryID}_sample{sampleID}_region{regionID}_slice{slice_type}_center{center_type}_v{date_info}.pickle'

    outputDir = os.path.join(data_out_prefix)
...
def main():
    ...
    print_to_log(log_path, "Data import finished!")
    
    bundles_df, annot_bundles_df = process_annotation(roi_df, annot_df, image_info[2])
    print("annot_bundles_df done!")
    print_to_log(log_path, "annot_bundles_df done!")
    
    time_start = time.time()
    image_norm = process_image(image, image_info[1], log_path, matching_info[3], scale_factor)
    time_end = time.time()
    time_dur = time_end - time_start
    print("image processed! total time: ", time_dur)
    print_to_log(log_path, "image processed! total time: " + str(time_dur))
    ...
    print_to_log(log_path, "image results generated!")
