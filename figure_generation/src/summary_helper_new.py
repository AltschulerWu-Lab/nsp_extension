...
# @Last Modified time: 2021-06-06 17:11:32
...
def plot_sample_variation_raw_measurements(annots_df_group, **kwargs):
	...
	for i_fig in range(num_subplots):
		...
		heel_coords = np.zeros((6,2, len(annots_df_current)))
		target_coords = np.zeros((6,2, len(annots_df_current)))
		...
		#### plot dots
		for i in range(heel_coords.shape[2]):
			for j in range(6):
				#### R cells
				ax.plot(heel_coords[j,0,i], heel_coords[j,1,i], 
						'o', color = color_code[j+1], markersize = 5, alpha = 0.5)
				
				#### L cells
				ax.plot(target_coords[j,0,i], target_coords[j,1,i], 
...
def plot_sample_variation_polar(annots_df_group, **kwargs):
	...
	for i_fig in range(num_subplots):
		...
		coords = {}
		for i in plot_color.keys():
			coords[i] = np.zeros((2, len(annots_df_current)))
		...
		for ind, bundle_no in enumerate(annots_df_current.index):
			...
			dTiCs = {3:pos_t3, 7:pos_t7, 4: pos_t4}
			target_grid_polar = get_target_grid_polar_summary(return_type = 'theory', dTiCs = dTiCs)
			coords['R3'][0, ind] = target_grid_polar[2,0]
			coords['R3'][1, ind] = annots_df_current.loc[bundle_no, 'R3']
			coords['R4'][0, ind] = target_grid_polar[5,0]
			coords['R4'][1, ind] = annots_df_current.loc[bundle_no, 'R4']
			coords['T3'][0, ind] = target_grid_polar[2,0]
			coords['T3'][1, ind] = annots_df_current.loc[bundle_no, 'T3c']
			coords['T7'][0, ind] = target_grid_polar[5,0]
			coords['T7'][1, ind] = annots_df_current.loc[bundle_no, 'T7c']
			coords['T4'][0, ind] = 0
			coords['T4'][1, ind] = 1
			
		### get centroids
		for t in coords.keys():
			sum_coords[t][:, i_sample] = np.mean(coords[t], axis = 1)
		...
		for ind in range(len(annots_df_current)):
			for t in ['R3', 'R4']:
				ax.plot(coords[t][0, ind], coords[t][1, ind], 
