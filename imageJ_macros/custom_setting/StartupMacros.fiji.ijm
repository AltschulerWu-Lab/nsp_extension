...
//custom short-cuts:
macro "Capture Image [q]" {
	run("Capture Image");
}

macro "Monitor Memory [0]" {
	run("Monitor Memory...");
}

macro "Collect Garbage [g]" {
	run("Collect Garbage");
}

macro "Switch channel 1 [1]"{
	Stack.getActiveChannels(channels);
	if(substring(channels, 0, 1) == '1'){
		new_channels = "0" + substring(channels, 1, 4);
	} else {
		new_channels = "1" + substring(channels, 1, 4);
	}
	Stack.setActiveChannels(new_channels);
}

macro "Switch channel 2 [2]"{
	Stack.getActiveChannels(channels);
	if(substring(channels, 1, 2) == '1'){
		new_channels = substring(channels, 0, 1) + "0" + substring(channels, 2, 4);
		
	} else {
		new_channels = substring(channels, 0, 1) + "1" + substring(channels, 2, 4);
	}
	Stack.setActiveChannels(new_channels);
}

macro "Switch channel 3 [3]"{
	Stack.getActiveChannels(channels);
	if(substring(channels, 2, 3) == '1'){
		new_channels = substring(channels, 0, 2) + "0" + substring(channels, 3, 4);
		
	} else {
		new_channels = substring(channels, 0, 2) + "1" + substring(channels, 3, 4);
	}
	Stack.setActiveChannels(new_channels);
}

macro "Switch channel 4 [4]"{
	Stack.getActiveChannels(channels);
	if(substring(channels, 3, 4) == '1'){
		new_channels = substring(channels, 0, 3) + "0" + substring(channels, 4, 5);
		
	} else {
		new_channels = substring(channels, 0, 3) + "1" + substring(channels, 4, 5);
	}
	Stack.setActiveChannels(new_channels);
}

macro "Update in ROI manager [u]"{
	roiManager("Update");
}
...
macro "Save_ROI [s]" {
	...
	date_string = toString(year) + toString(month+1) + toString(dayOfMonth);
	time_string = toString(hour) + toString(minute) + toString(second);
	output_directory=getDirectory("Choose output directory");
	csv_name = substring(image_name,0,indexOf(image_name, ".tif")) + "_ROI_" + date_string + "_" + time_string + ".csv";
	roi_name = substring(image_name,0,indexOf(image_name, ".tif")) + "_ROI_" + date_string + "_" + time_string + ".zip";
	csv_path = output_directory + csv_name;
	roi_path = output_directory + roi_name;
